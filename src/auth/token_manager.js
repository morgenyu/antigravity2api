...
class TokenManager {
  ...
  syncTokensFromEnv() {
    const envTokens = process.env.TOKENS_JSON;
    if (!envTokens) return;

    try {
      const parsed = JSON.parse(envTokens);
      if (!Array.isArray(parsed) || parsed.length === 0) {
        log.warn('TOKENS_JSON ?????? token ????????');
        return;
      }

      let existing = [];
      if (fs.existsSync(this.filePath)) {
        const fileData = fs.readFileSync(this.filePath, 'utf8').trim();
        existing = fileData ? JSON.parse(fileData) : [];
      }

      if (existing.length > 0) {
        log.info('????? accounts.json??? TOKENS_JSON ???');
        return;
      }

      fs.writeFileSync(this.filePath, JSON.stringify(parsed, null, 2), 'utf8');
      log.info(`??????? TOKENS_JSON ??? ${parsed.length} ? token`);
    } catch (error) {
      log.error('?? TOKENS_JSON ??:', error.message);
    }
  }
